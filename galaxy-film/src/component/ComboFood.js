...
import { createSearchParams, generatePath, Link, useNavigate } from "react-router-dom/dist";
...
useEffect(() => {
  const totalAdult=
  setTotalAdult(quantityadult*100000)
  
}, [quantityadult])
// total quantity Member
useEffect(() => {
  const totalMember=
  setTotalMember(quantityMember*80000)
  
}, [quantityMember])
// total quantity Double Member
useEffect(() => {
  const totalDoubleMember=
  setTotalDoubleMember(quantityDoubleMember*200000)
  
}, [quantityDoubleMember])
useEffect(() => {
  const totalDouble=
  setTotalDouble(quantityDouble*180000)
  
}, [quantityDouble])
// total food
useEffect(() => {
  const totalC1=
  setTotalC1(quantityCombo1*80000)
  
}, [quantityCombo1])
useEffect(() => {
  const totalC2=
  setTotalC2(quantityCombo2*60000)
  
}, [quantityCombo2])
useEffect(() => {
  const totalC3=
  setTotalC3(quantityCombo3*40000)
  
}, [quantityCombo3])
useEffect(() => {
  const totalC4=
  setTotalC4(quantityCombo4*20000)
  
}, [quantityCombo4])
useEffect(() => {
  const Total=
  setTotalTicket(totalAdult+totalMember + totalDouble + totalDoubleMember)
  
}, [totalAdult, totalMember, totalDouble, totalDoubleMember])
useEffect(() => {
  const Total=
  setTotalFood(totalC1+totalC2 + totalC3 + totalC4)
  
}, [totalC1, totalC2, totalC3, totalC4])

useEffect(() => {
  const Total=
  setTotalCart(TotalTicket+TotalFood)
          ...
          <div className="col-3">
            ...
            <Card style={{ width: "18rem", margin: "20px" }}>
              ...
              <ListGroup className="list-group-flush">
                ...
                <span className="text-end text-danger py-1 fs-5 px-1 "> {TotalCart} VND </span>
